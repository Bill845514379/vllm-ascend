...
class TestMiniMaxM3Modeling(unittest.TestCase):
    def test_moe_passes_swigluoai_config_during_construction(self) -> None:
        ...
        with (
            patch(
                "vllm_ascend.models.minimax_m3.modeling.get_tensor_model_parallel_world_size",
                return_value=1,
            ),
            patch(
                "vllm_ascend.models.minimax_m3.modeling.MiniMaxM3MLP",
                return_value=nn.Identity(),
            ),
            patch(
                "vllm_ascend.models.minimax_m3.modeling.GateLinear",
                return_value=gate,
            ),
            patch(
                "vllm_ascend.models.minimax_m3.modeling.FusedMoE",
                return_value=nn.Identity(),
            ) as fused_moe,
        ):
